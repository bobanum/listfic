...
//TODO Enlever "source.php" lors de la création du final
...
Dossier_abstract::init();
abstract class Dossier_abstract {
	...
	/**
	 * RÉVISER
	 * Est appelé avant le chargement proprement dit. Permet de déterminer des variables telles la racine du site en fonction de la page d'appel.
	 */
	static public function init() {

		//$racineRelative = "..";
		//static::$racine = realpath(dirname(__FILE__)."/".$racineRelative);
		static::$racine = realpath('.');
	}
	/**
...
//				var_dump($fct);
	...
	public function ligneProjet($admin=false){
		$resultat = '';
		$resultat .= $this->affichageEcran();
		if ($admin) $resultat .= $this->boutonsAdmin();
		if ($this->prefixe) $resultat .= $this->prefixe. " : ";
		$resultat .= Fct_titre::html($this);
		$liens = $this->creerLiens();
		if (count($liens)) $resultat .= ' <span class="liens">'.implode("", $liens).'</span>';
		return $resultat;
	}
	...
	public function boutonsAdmin(){
		...
		$resultat .= '<a style="font-size: 150%; line-height: 0; position: relative; text-decoration: none; top: 0.21em;" href="?admin&a='.urlencode($this->url).'#modifier">&#x270D;</a>';
		/*$resultat .= Fct_visible::html_bouton($this);
		$resultat .= Fct_fichiers::html_bouton($this);
		$resultat .= Fct_solution::html_bouton($this);*/
		$resultat .= implode('', $this->executerFct('html_bouton'));
		$resultat .= '</span>';
		return $resultat;
	}
	/**
